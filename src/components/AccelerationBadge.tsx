...
export function AccelerationBadge({
  ...
  const active = records.some((r) => r.state === "active"),
    software = records.some((r) => r.state === "software");
  const label = active
    ? "GPU active"
    : software
      ? "CPU encoding"
      ...
      <Tooltip.Root>
        <Tooltip.Trigger asChild>
          <button
            ...
            aria-label={label}
          >
            {active ? <Gauge size={14} /> : <Cpu size={14} />} {label}
          </button>
        </Tooltip.Trigger>
        <Tooltip.Portal>
          <Tooltip.Content
            sideOffset={6}
            className="z-50 max-w-xs rounded-md border bg-card p-3 text-xs shadow-xl"
          >
            {records.map((r, i) => (
              <div key={`${r.component}-${i}`}>
                <b>{r.component.replace("_", " ")}:</b> {r.state}
